...
Un **Bot de traduction relais** qui permet de lire et d'écrire des messages Discord dans la “langue de votre choix”, avec une **UI de bureau (Tkinter)** pour le contrôler depuis votre appareil, et un **Console** utilisable depuis le terminal. (Prise en charge actuelle au 2025/09/08 : en, ja, zh, ko, es, fr, de, it, pt, ru, id, vi, th) Les journaux de traduction peuvent être enregistrés au format **JSONL**, et un chemin de partage UNC (ex : `\\raspberrypi\IMTB-D\messages.jsonl`) peut également être spécifié.
...
- **Console** : Liaison et envoi depuis le terminal. Affichage des journaux en mode tail.
...
# Pour une utilisation locale, localhost est recommandé (l'UI démarre Relay automatiquement)
...
> Lors de l'utilisation de UNC sur Linux/mac, il est préférable de le monter au préalable et de spécifier un chemin normal. ※IMTBD_JSONL_PATH est **sensible à la casse** (Linux)
...
- Si `IMTBD_API_BASE` est `http://127.0.0.1:8765` ou `localhost`,  
  ...
  Modifiez le fichier `.env` depuis l'onglet « Setup » et **Save .env**.

- Dans l'onglet « Destinations », **Bind** la destination (DM/Channel) → saisissez le texte → **Send**.
  
  ![destinations.png](docs/images/destinations.png)

- Les envois et réceptions seront reflétés dans le journal en bas.
  
  - En ayant sélectionné la destination (DM/Channel), cliquez sur « Open Window » → une fenêtre de chat individuelle s'ouvrira.
  ...
  - Traduction d'image (Inpaint)
    ...
    - Glissez-déposez une image pour effectuer une traduction d'image par inpainting.
    
    - À ce stade, ce n'est pas très propre, mais cela peut servir de référence.
...
- Modifiez le fichier `.env` de l'UI pour que `IMTBD_API_BASE` soit `http://<server-ip>:8765`.  
- Dans ce cas, le démarrage/arrêt de l'UI sera désactivé, et elle fonctionnera en **mode distant**.
...
# Tapez directement dans l'entrée standard pour envoyer (les journaux s'affichent en mode tail).
```
...
- Par défaut : `log/messages.jsonl`. Le chemin de sauvegarde peut être modifié dans `.env` avec `IMTBD_JSONL_PATH`.  
- L'UI suit ce fichier pour l'affichage à l'écran. Il est également accessible via un partage UNC.
