...
This tool combines a **Translation Relay Bot (Relay)** that can translate Discord messages into your “preferred language,” a **Desktop UI (Tkinter)** for local operation, and a **Console** for terminal use.  
Translation logs can be saved in **JSONL** format, and a UNC shared path (e.g., `\\raspberrypi\IMTB-D\messages.jsonl`) can also be specified.

- **Relay**: Discord Bot and local HTTP API (`/bind`, `/send`, `/send_image`, `/stats`)

- **Relay r3 addition**: **`/translate`** (a generic API that receives text via HTTP and returns the translation via HTTP)

- **UI**: Edit .env, register and send destinations, view logs, **file translation (live preview)**, automatic Relay startup when running locally

- **Console**: Bind & send from the terminal. Tail display of logs

> Requirements: **Discord Bot Token** and **OpenAI API Key** (a configuration that does not directly hit OpenAI is also acceptable)
...
- [Usage](#usage)
  ...
  - [A. Using UI + Relay Locally](#a-using-ui--relay-locally)
  
  - [B. Connecting to Remote Relay](#b-connecting-to-remote-relay)
  
  - [C. Console (Terminal)](#c-console-terminal)

- [API (Relay)](#api-relay)
  
  - [/translate (New r3)](#translate-new-r3)
...
- Python 3.10+ (an environment where Tkinter can be used)
...
- The log at the bottom will reflect the sent and received messages.
  ...
  - With the destination (DM/Channel) selected, click "Open Window" → an individual chat window will open.
  ...
  - Text Translation
    ...
    - If multiple lines are needed, you can press Ctrl+Enter for a line break.
  
  - Image Translate (Inpaint)
    
    - Drag and drop an image to perform inpainting-style image translation.
    
    - Currently, the results may not be very clean, but it can serve as a reference.
...
### B. Connecting to Remote Relay (e.g., Raspberry Pi)
...
**A generic API that translates text received via HTTP and returns the translation via HTTP**. It does not go through Discord.
...
- When `ok` is `true`, `translated` will contain the translation.
...
- `POST /bind` — Register the console name and destination (dm/channel, id, lang, etc.)
...
- `GET /stats` — Startup status and binding list> `/translate` is **ideal for direct replies to HTTP clients**, making it suitable for integration with external tools like VS Code. The traditional flow via Discord uses `/bind` and `/send`.
...
- The UI tails this file for display. It can be viewed even over UNC shares.
...
A: `localhost` may be resolving to IPv6 and unable to connect. Please try **`127.0.0.1`**. Use `<server-ip>` for remote connections.

**Q: Permission denied (writing `console_routes.json`)**  
A: This may occur if the editor has the file open (exclusive) or due to Windows Controlled Folder Access. Change the save location to the user directory or close the editor and try again.
...
- **Git Usage**
  ...
  - Generally ignore `.vscode/`. If sharing, only include the minimum necessary without secrets, such as `extensions.json`/`tasks.json`.
