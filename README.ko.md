...
Discord의 메시지를 “좋아하는 언어”로 번역하여 읽고 쓸 수 있는 **번역 릴레이 Bot(릴레이)**,  
그것을 손에서 조작할 수 있는 **데스크탑 UI(Tkinter)**, 터미널에서 사용할 수 있는 **콘솔**을 종합한 도구입니다. (2025/09/08 현재 지원: en, ja, zh, ko, es, fr, de, it, pt, ru, id, vi, th)  
번역 로그는 **JSONL** 형식으로 저장할 수 있으며, UNC 공유 경로(예: `\\raspberrypi\IMTB-D\messages.jsonl`)도 지정 가능합니다.

- **릴레이**: Discord Bot과 로컬 HTTP API(`/bind`, `/send`, `/send_image`, `/stats`).
- **UI**: .env 편집, 수신처 등록 및 전송, 로그 조회, **파일 번역(실시간 미리보기)**, 로컬에서는 릴레이 자동 시작.
- **콘솔**: 터미널에서 바인드 및 전송. 로그의 tail 표시.
...
# 로컬에서 사용할 경우 localhost를 권장(UI가 릴레이를 자동 시작)
...
> Linux/mac에서 UNC를 사용할 경우, 미리 마운트하여 일반 경로로 지정하는 것이 확실합니다. ※IMTBD_JSONL_PATH는 **대소문자를 구분**합니다(Linux).

---

## 사용 방법

### A. 로컬에서 UI + 릴레이 사용하기(최단)
...
- `IMTBD_API_BASE`가 `http://127.0.0.1:8765` 또는 `localhost`인 경우,  
  UI가 **릴레이의 시작을 자동으로 보조**합니다(시작 후 "API ready"가 표시됨).
...
- 하단의 로그에 송수신이 반영됩니다.
  ...
  - 이미지 번역(Inpaint)
    ...
    - 이미지를 D&D하여 인페인팅 방식의 이미지 번역을 수행합니다.
    
    - 현재 단계에서는 그리 깔끔하지 않지만, 참고용으로는 됩니다.
...
### B. 원격 릴레이(예: Raspberry Pi)에 연결

- 서버(Pi 등)에서 `IMTB-D_relay.py`를 실행해 두고,
- UI 쪽의 `.env`에서 `IMTBD_API_BASE`를 `http://<server-ip>:8765`로 설정.  
...
- `POST /send` — 지정된 콘솔로 텍스트 전송(`lang`으로 일시적으로 덮어쓸 수 있음)  
- `POST /send_image` — 이미지의 OCR → 번역 → 인페인팅 및 그리기 → 전송  
- `GET  /stats` — 실행 상태 및 바인딩 목록

---

## 로그(JSONL)

- 기본값: `log/messages.jsonl`. `.env`의 `IMTBD_JSONL_PATH`로 저장 위치를 변경할 수 있습니다.  
