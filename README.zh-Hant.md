...
Discord 的消息可以翻譯成 “喜歡的語言” 來讀取和寫入的 **翻譯中繼 Bot（Relay）**，  
以及可以從手邊操作的 **桌面 UI（Tkinter）**，和可以從終端使用的 **Console** 組成的工具。(截至 2025/09/08 支援語言：en, ja, zh, ko, es, fr, de, it, pt, ru, id, vi, th)  
...
console_routes.json      # 目的地的保存（UI 寫入）
...
# 使用本地時建議使用 localhost（UI 會自動啟動 Relay）
IMTBD_API_BASE=http://127.0.0.1:8765

# （可選）日誌的保存位置。Windows 使用 UNC，Linux/mac 可以使用常規路徑
...
> 在 Linux/mac 使用 UNC 時，提前掛載並使用常規路徑指定是最可靠的。※IMTBD_JSONL_PATH 是 **區分大小寫**（Linux）
...
- 當 `IMTBD_API_BASE` 為 `http://127.0.0.1:8765` 或 `localhost` 時，  
  UI 會 **自動協助啟動 Relay**（啟動後顯示「API ready」）。
  
  ![setup.png](docs/images/setup.png)
  
  從「Setup」選項卡編輯 `.env` 並 **保存 .env**。

- 在「Destinations」選項卡中綁定目的地（DM/Channel）→ 輸入文本 → **發送**。
...
- 在這種情況下，UI 的 Start/Stop 將被禁用，並作為 **遠程模式** 運行。
...
- `POST /send_image` — 圖像的 OCR → 翻譯 → 修補和繪製 → 發送  
- `GET  /stats` — 啟動狀態和綁定列表
...
- 預設：`log/messages.jsonl`。可以通過 `.env` 的 `IMTBD_JSONL_PATH` 來更改保存位置。  
- UI 會 tail 此文件並顯示在畫面上。即使通過 UNC 共享也可以查看。

---

## 常見問題（FAQ）

**Q: Windows 的 UNC 路徑該怎麼寫？**  
A: 在 `.env` 中應該用 **兩個反斜杠** 表示 `\\raspberrypi\IMTB-D\messages.jsonl`。  
   由於 `.env` 中的轉義，實際上應該寫成 `\\\\raspberrypi\\IMTB-D\\messages.jsonl` 比較安全。
